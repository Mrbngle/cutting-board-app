...
((e, t) => {
		...
		(this._handle = null),
			...
			(this.parseChunk = function (t, e) {
				...
				this.isFirstChunk &&
					...
					void 0 !== (r = this._config.beforeFirstChunk(t)) &&
					(t = r),
					(this.isFirstChunk = !1),
					(this._halted = !1);
				...
				if (!this._handle.paused() && !this._handle.aborted()) {
					(t = r.meta.cursor),
						(i =
							(this._finished ||
								((this._partialLine = i.substring(t - this._baseIndex)), (this._baseIndex = t)),
							r && r.data && (this._rowCount += r.data.length),
							this._finished || (this._config.preview && this._rowCount >= this._config.preview)));
					if (a) n.postMessage({ results: r, workerId: v.WORKER_ID, finished: i });
					else if (U(this._config.chunk) && !e) {
						if (
							(this._config.chunk(r, this._handle), this._handle.paused() || this._handle.aborted())
						)
					...
					return (
						this._config.step ||
							this._config.chunk ||
							((this._completeResults.data = this._completeResults.data.concat(r.data)),
			...
			(this._sendError = function (e) {
				U(this._config.error)
					...
					: a &&
						this._config.error &&
						n.postMessage({ workerId: v.WORKER_ID, error: e, finished: !1 });
			});
	}
	function f(e) {
		var r;
			...
			(this._readChunk = function () {
				if (this._finished) this._chunkLoaded();
					...
					if (
						...
						s || ((r.onload = y(this._chunkLoaded, this)), (r.onerror = y(this._chunkError, this))),
						r.open(this._config.downloadRequestBody ? 'POST' : 'GET', this._input, !s),
		...
		(this.stream = function (e) {
			...
			(this._readChunk = function () {
				var e = this._input,
					t =
						(this._config.chunkSize &&
							((t = Math.min(this._start + this._config.chunkSize, this._input.size)),
							(e = r.call(e, this._start, t))),
						i.readAsText(e, this._config.encoding));
			...
			(this._nextChunk = function () {
				...
				if (!this._finished)
					return (
						...
						(i = e ? ((t = i.substring(0, e)), i.substring(e)) : ((t = i), '')),
						(this._finished = !i),
						this.parseChunk(t)
					);
			});
	...
	function i(m) {
		...
		function g() {
			if (
				(p &&
					...
					(k(
						...
						"Unable to auto-detect delimiting character; defaulted to '" + v.DefaultDelimiter + "'"
					),
					(a = !1)),
				m.skipEmptyLines &&
						...
						s = ((e, t) =>
							((e) => (
								m.dynamicTypingFunction &&
									void 0 === m.dynamicTyping[e] &&
									(m.dynamicTyping[e] = m.dynamicTypingFunction(e)),
								!0 === (m.dynamicTyping[e] || m.dynamicTyping)
							))(e)
								? 'true' === t ||
									'TRUE' === t ||
									('false' !== t &&
										...
										(((e) => {
											...
											: d.test(t)
												...
												: '' === t
													...
													: t))
								: t)(
							(n = m.header ? (r >= c.length ? '__parsed_extra' : c[r]) : n),
							(s = m.transform ? m.transform(s, n) : s)
						);
					...
					m.header &&
						(r > c.length
							? k(
									'FieldMismatch',
									'TooManyFields',
									'Too many fields: expected ' + c.length + ' fields but parsed ' + r,
									f + t
								)
							: r < c.length &&
								k(
									'FieldMismatch',
									'TooFewFields',
									'Too few fields: expected ' + c.length + ' fields but parsed ' + r,
									f + t
								)),
					i
				);
			...
			var r;
			p &&
				(m.header || m.dynamicTyping || m.transform) &&
				((r = 1),
				!p.data.length || Array.isArray(p.data[0])
					? ((p.data = p.data.map(i)), (r = p.data.length))
					: (p.data = i(p.data, 0)),
				m.header && p.meta && (p.meta.fields = c),
				(f += r));
		}
		function _() {
				...
				(p = e),
					_()
						...
						: (g(),
							0 !== p.data.length &&
								((r += e.data.length),
								m.preview && r > m.preview ? s.abort() : ((p.data = p.data[0]), t(p, i))));
			})),
			(this.parse = function (e, t, i) {
				var r = m.quoteChar || '"',
					r =
						...
						(a = !1),
						m.delimiter
							? U(m.delimiter) && ((m.delimiter = m.delimiter(e)), (p.meta.delimiter = m.delimiter))
							: ((r = ((e, t, i, r, n) => {
									var s, a, o, h;
									...
									for (var u = 0; u < n.length; u++) {
										for (
											var d,
												...
												g =
													((o = void 0),
													new E({ comments: r, delimiter: f, newline: t, preview: 10 }).parse(e)),
												_ = 0;
											_ < g.data.length;
											_++
										)
											i && y(g.data[_])
												...
												: ((d = g.data[_].length),
													(c += d),
													void 0 === o ? (o = d) : 0 < d && ((l += Math.abs(d - o)), (o = d)));
								...
								})(e, m.newline, m.skipEmptyLines, m.comments, m.delimitersToGuess)).successful
									? (m.delimiter = r.bestDelimiter)
									: ((a = !0), (m.delimiter = v.DefaultDelimiter)),
								(p.meta.delimiter = m.delimiter)),
						w(m));
				return (
					...
					(p = s.parse(n, t, i)),
					g(),
					l ? { meta: { paused: !0 } } : p || { meta: { paused: !1 } }
				);
			}),
	...
	function E(C) {
		var S = (C = C || {}).delimiter,
			...
			j = F;
		if (
			(void 0 !== C.escapeChar && (j = C.escapeChar),
			('string' != typeof S || -1 < v.BAD_DELIMITERS.indexOf(S)) && (S = ','),
			x === S)
		)
				...
				if (i[z] === F)
					for (m = z, z++; ; ) {
						...
						if (F === j && i[m + 1] === j) m++;
							...
							var y = v(
								-1 === (g = -1 !== g && g < m + 1 ? i.indexOf(O, m + 1) : g) ? p : Math.min(p, g)
							);
	...
	return (
		(v.parse = function (e, t) {
			...
			if (
				...
				return (
					...
					v.NODE_STREAM_INPUT,
					'string' == typeof e
						? ((e = ((e) => (65279 !== e.charCodeAt(0) ? e : e.slice(1)))(e)),
							(i = new (t.download ? f : c)(t)))
						: !0 === e.readable && U(e.read) && U(e.on)
							? (i = new p(t))
							: ((n.File && e instanceof File) || e instanceof Object) && (i = new l(t)),
					i.stream(e)
				);
			((i = (() => {
				var e;
						...
						if ('object' == typeof t) {
							if (
								('string' != typeof t.delimiter ||
									v.BAD_DELIMITERS.filter(function (e) {
		...
		(v.BYTE_ORDER_MARK = '\ufeff'),
		(v.BAD_DELIMITERS = ['\r', '\n', '"', v.BYTE_ORDER_MARK]),
		(v.WORKERS_SUPPORTED = !s && !!n.Worker),
		...
		(v.FileStreamer = l),
		(v.StringStreamer = c),
		(v.ReadableStreamStreamer = p),
		n.jQuery &&
			((d = n.jQuery).fn.parse = function (o) {
				var i = o.config || {},
					h = [];
				return (
			...
			(n.onmessage = function (e) {
				...
				'string' == typeof e.input
					...
					: ((n.File && e.input instanceof File) || e.input instanceof Object) &&
						(e = v.parse(e.input, e.config)) &&
						n.postMessage({ workerId: v.WORKER_ID, results: e, finished: !0 });
			}),
