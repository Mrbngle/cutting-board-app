...
function createSettingsStore(): SettingsStore {
  ...
  const { subscribe, set, update }: Writable<CutSettings> =
    createPersistentStore<CutSettings>(STORAGE_KEY, defaultSettings);
  ...
  const updateSetting = <K extends keyof CutSettings>(settingName: K, value: any): void => {
    ...
    update((currentState: CutSettings): CutSettings => {
      ...
      const dimensionKeysMm: Array<keyof CutSettings> = ['kerfMm', 'edgeTrimMm', 'minScrapWidthMm', 'minScrapLengthMm'];
      let currentUnits: 'mm' | 'inches' | undefined;
      if (dimensionKeysMm.includes(settingName)) { currentUnits = get(boardStore).units; }
      ...
      try {
        if (dimensionKeysMm.includes(settingName)) {
          ...
          newState[settingName as 'kerfMm' | 'edgeTrimMm' | 'minScrapWidthMm' | 'minScrapLengthMm'] = valueMm;
        } else if (settingName === 'respectGrain') {
          newState.respectGrain = Boolean(value);
        } else if (settingName === 'optimizationAlgo') {
          if (value === 'waste' || value === 'cuts' || value === 'priority') { newState.optimizationAlgo = value; }
          else { console.warn(`SettingsStore: Invalid value "${value}" provided for optimizationAlgo.`); return currentState; }
        } else { console.warn(`SettingsStore: Unknown setting "${String(settingName)}"`); return currentState; }
      } catch (error) { console.error(`SettingsStore: Error updating "${String(settingName)}":`, error); return currentState; }
      return newState;
    });
  };
