...
type PieceInputData = Partial<
	Omit<CutPiece, 'id' | 'widthMm' | 'lengthMm' | 'width' | 'length' | 'quantity'>
> & { width: number | string; length: number | string; quantity?: number | string };
type PieceUpdateData = Partial<
	Omit<CutPiece, 'id' | 'widthMm' | 'lengthMm' | 'width' | 'length'>
> & { width?: number | string; length?: number | string; quantity?: number | string };
...
function createPiecesStore(): PiecesStore {
	...
	const { subscribe, set, update }: Writable<CutPiece[]> = createPersistentStore<CutPiece[]>(
		STORAGE_KEY,
		defaultPiecesState
	);
	...
	const addPiece = (pieceData: PieceInputData): boolean => {
		...
		if (isNaN(inputWidth) || isNaN(inputLength) || inputWidth <= 0 || inputLength <= 0) {
			console.error('piecesStore.addPiece: Invalid dimensions provided.', {
				inputWidth,
				inputLength
			});
		...
		const name = String(
			pieceData.name || `Piece ${widthMm.toFixed(0)}x${lengthMm.toFixed(0)}`
		).trim();
		const grainDirection = pieceData.grainDirection || 'none';
		if (!['length', 'width', 'none'].includes(grainDirection)) {
			/* warning */
		}
		const id = crypto.randomUUID();
		const newPiece: CutPiece = {
			id,
			name: name || `Piece ${id.substring(0, 4)}`,
			widthMm,
			lengthMm,
			quantity,
			priority,
	...
	const removePiece = (id: string): void => {
		update((currentPieces: CutPiece[]): CutPiece[] => {
			...
			if (initialLength === filteredPieces.length) {
				console.warn(`piecesStore.removePiece: Piece with ID ${id} not found.`);
			}
	...
	const updatePiece = (id: string, updatedData: PieceUpdateData): boolean => {
		...
		update((currentPieces: CutPiece[]): CutPiece[] => {
			return currentPieces.map((piece: CutPiece): CutPiece => {
				if (piece.id === id) {
					...
					if (updatedData.width !== undefined) {
						/* conversion logic */
					}
					if (updatedData.length !== undefined) {
						/* conversion logic */
					}
					if (updatedData.name !== undefined) {
						updatedPiece.name = String(updatedData.name).trim() || `Piece ${id.substring(0, 4)}`;
					}
					if (updatedData.quantity !== undefined) {
						updatedPiece.quantity = Math.max(1, Math.round(Number(updatedData.quantity)) || 1);
					}
					if (updatedData.priority !== undefined) {
						updatedPiece.priority = Math.max(
							1,
							Math.min(5, Math.round(Number(updatedData.priority)) || 5)
						);
					}
					if (
						updatedData.grainDirection !== undefined &&
						['length', 'width', 'none'].includes(updatedData.grainDirection)
					) {
						updatedPiece.grainDirection = updatedData.grainDirection;
					}
					return updatedPiece;
				}
				return piece;
		...
		if (!pieceFound) {
			console.warn(`piecesStore.updatePiece: Piece with ID ${id} not found.`);
		}
		return pieceFound;
	};
