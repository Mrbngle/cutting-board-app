    ...
    try {
      ...
      if (storedJson) {
        try {
          ...
          console.error(`PersistentStore: Error parsing JSON for key "${key}". Using initial value.`, e);
      ...
      console.warn(`PersistentStore: localStorage not available or writable for key "${key}". Persistence disabled.`, e);
  ...
  if (isBrowserAndStorageAvailable) {
    store.subscribe(($currentValue: T) => {
      try {
        ...
        console.error(`PersistentStore: Failed to save state for key "${key}" to localStorage. (Quota exceeded?)`, e);
      }
    });
  }
